...
def hmm_xgb_pipeline(train_df, test_df, params):
    ...
    long_th = params.get("long_threshold", 0.55)
    short_th = params.get("short_threshold", 0.45)
    ...
    test_df.loc[
        (test_df["regime"] == "bull") &
        (test_df["xgb_prob"] > long_th),
        "signal"
    ] = 1

    test_df.loc[
        (test_df["regime"] == "bear") &
        (test_df["xgb_prob"] < short_th),
        "signal"
    ] = -1
...
def go_hmm_xgb_pipeline(train_df, test_df, params):
...
def perfect_foresight_pipeline(train_df, test_df, params):
    ...
    future_return = test_df["return_1"].shift(-1)
